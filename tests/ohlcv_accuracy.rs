...
fn test_ohlcv_accuracy_hand_computed_values() {
    ...
    agg.feed(&make_tick("BTC-USD", dec!(100), dec!(5), 60_000))
        .unwrap();
    agg.feed(&make_tick("BTC-USD", dec!(110), dec!(3), 60_100))
        .unwrap();
    agg.feed(&make_tick("BTC-USD", dec!(90), dec!(2), 60_200))
        .unwrap();
    agg.feed(&make_tick("BTC-USD", dec!(105), dec!(4), 60_300))
        .unwrap();
...
fn test_ohlcv_accuracy_single_tick_bar() {
    ...
    agg.feed(&make_tick("ETH-USD", dec!(3000), dec!(7), 30_000))
        .unwrap();
...
fn test_ohlcv_accuracy_consecutive_bar_open_close_relationship() {
    ...
    agg.feed(&make_tick("BTC-USD", dec!(50000), dec!(1), 60_000))
        .unwrap();
    agg.feed(&make_tick("BTC-USD", dec!(51000), dec!(1), 60_500))
        .unwrap(); // close of bar 1

    // Bar 2 first tick closes bar 1
    let bars = agg
        .feed(&make_tick("BTC-USD", dec!(52000), dec!(1), 120_000))
        .unwrap();
    let bar1 = &bars[0];
...
fn test_ohlcv_accuracy_volume_decimal_precision() {
    ...
    assert_eq!(
        ...
        "volume decimal precision incorrect"
    );
...
fn test_ohlcv_accuracy_5min_boundary_alignment() {
    ...
    agg.feed(&make_tick("BTC-USD", dec!(100), dec!(1), 300_000))
        .unwrap();
    agg.feed(&make_tick("BTC-USD", dec!(110), dec!(1), 599_999))
        .unwrap();
    ...
    let completed = agg
        .feed(&make_tick("BTC-USD", dec!(120), dec!(1), 600_000))
        .unwrap();
