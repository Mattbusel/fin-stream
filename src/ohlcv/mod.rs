...
impl OhlcvBar {
    ...
    /// Bar close-low trend: OLS slope of (close - low) per bar.
    pub fn bar_close_low_trend(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 2 { return None; }
        let vals: Vec<f64> = bars.iter()
            .map(|b| (b.close - b.low).to_f64().unwrap_or(0.0))
            .collect();
        let n = vals.len() as f64;
        let x_mean = (n - 1.0) / 2.0;
    ...
    /// Open body skew: mean of (open - (open+close)/2) / |close-open| per bar.
    pub fn open_body_skew(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let body = (b.close - b.open).abs().to_f64()?;
            if body == 0.0 { return None; }
            let mid = ((b.open + b.close) / rust_decimal::Decimal::TWO).to_f64()?;
            let open = b.open.to_f64()?;
            Some((open - mid) / body)
        }).collect();
        if vals.is_empty() { return None; }
        Some(vals.iter().sum::<f64>() / vals.len() as f64)
    ...
    /// Bar volume trend ratio: fraction of bars with volume above previous bar's volume.
    pub fn bar_volume_trend_ratio(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 2 { return None; }
        let ups = bars.windows(2)
            .filter(|w| w[1].volume.to_f64().unwrap_or(0.0) > w[0].volume.to_f64().unwrap_or(0.0))
            .count() as f64;
        Some(ups / (bars.len() - 1) as f64)
    }

    /// Bar body-range ratio: mean of |close - open| / (high - low) per bar.
    pub fn bar_body_range_ratio(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let range = (b.high - b.low).to_f64()?;
    ...
    /// Fraction of bars where close > open (bullish bars).
    pub fn bar_close_above_open_ratio(bars: &[OhlcvBar]) -> Option<f64> {
        if bars.is_empty() { return None; }
        let bullish = bars.iter().filter(|b| b.close > b.open).count();
        Some(bullish as f64 / bars.len() as f64)
    }

    /// Mean second-order change of bar highs (acceleration of highs).
    pub fn bar_high_acceleration(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 3 { return None; }
        let highs: Vec<f64> = bars.iter().filter_map(|b| b.high.to_f64()).collect();
        if highs.len() < 3 { return None; }
        let accels: Vec<f64> = highs.windows(3)
            .map(|w| (w[2] - w[1]) - (w[1] - w[0]))
            .collect();
        Some(accels.iter().sum::<f64>() / accels.len() as f64)
    }

    /// Mean spread between low and open across bars.
    pub fn low_open_spread(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter()
            .filter_map(|b| (b.open - b.low).to_f64())
            .collect();
        if vals.is_empty() { return None; }
        Some(vals.iter().sum::<f64>() / vals.len() as f64)
    }

    /// Mean ratio of close to open across bars (close/open).
    pub fn close_over_open(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let o = b.open.to_f64()?;
    ...
    /// Standard deviation of close prices across bars.
    pub fn bar_close_dispersion(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 2 { return None; }
        let closes: Vec<f64> = bars.iter().filter_map(|b| b.close.to_f64()).collect();
    ...
    /// Mean of (open + close) / 2 across bars.
    pub fn bar_open_close_mean(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
    ...
    pub fn close_gap_from_prior(bars: &[OhlcvBar]) -> Option<f64> {
        ...
        let gaps: Vec<f64> = bars.windows(2).filter_map(|w| {
            (w[1].close - w[0].close).to_f64()
        }).collect();
        if gaps.is_empty() { return None; }
        Some(gaps.iter().sum::<f64>() / gaps.len() as f64)
    }

    /// Mean volume per bar (volume / number of bars — same as mean volume but explicit).
    pub fn bar_volume_per_bar(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
    ...
    /// Skewness of (high - low) ranges across bars.
    pub fn bar_range_skew(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 3 { return None; }
        let ranges: Vec<f64> = bars.iter().filter_map(|b| (b.high - b.low).to_f64()).collect();
        if ranges.len() < 3 { return None; }
        let n = ranges.len() as f64;
        let mean = ranges.iter().sum::<f64>() / n;
        let var = ranges.iter().map(|r| (r - mean).powi(2)).sum::<f64>() / n;
        let std = var.sqrt();
        if std == 0.0 { return None; }
        Some(ranges.iter().map(|r| ((r - mean) / std).powi(3)).sum::<f64>() / n)
    }

    /// Mean fraction of bar range from low to open (where open sits in the range).
    pub fn bar_open_mid_fraction(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let range = (b.high - b.low).to_f64()?;
    ...
    /// Mean of (high - close) gaps across bars.
    pub fn bar_high_close_gap(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let gaps: Vec<f64> = bars.iter().filter_map(|b| (b.high - b.close).to_f64()).collect();
        if gaps.is_empty() { return None; }
        Some(gaps.iter().sum::<f64>() / gaps.len() as f64)
    }

    /// Linear trend slope of open prices across bars.
    pub fn bar_open_trend(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 2 { return None; }
        let opens: Vec<f64> = bars.iter().filter_map(|b| b.open.to_f64()).collect();
        ...
        let x_mean = (n - 1.0) / 2.0;
        let y_mean = opens.iter().sum::<f64>() / n;
        let num: f64 = opens.iter().enumerate().map(|(i, &y)| (i as f64 - x_mean) * (y - y_mean)).sum();
        let den: f64 = opens.iter().enumerate().map(|(i, _)| (i as f64 - x_mean).powi(2)).sum();
        if den == 0.0 { return Some(0.0); }
        Some(num / den)
    }
    ...
    /// Mean of (close - (high + low) / 2) — how far close is from range midpoint.
    pub fn bar_close_vs_range_mid(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let mid = ((b.high + b.low) / rust_decimal::Decimal::TWO).to_f64()?;
    ...
    /// Average True Range proxy: mean of max(high-low, |close-prior_close|) across bars.
    pub fn bar_atr_proxy(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 2 { return None; }
        let trs: Vec<f64> = bars.windows(2).filter_map(|w| {
            let hl = (w[1].high - w[1].low).to_f64()?;
            let cp = (w[1].close - w[0].close).abs().to_f64()?;
            Some(hl.max(cp))
        }).collect();
    ...
    /// Mean of (high + low) / 2 - close: how much close lags bar midpoint.
    pub fn bar_mid_close_diff(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let mid = ((b.high + b.low) / rust_decimal::Decimal::TWO).to_f64()?;
    ...
    /// Candle strength: mean of body / range ratio across bars (0 = pure wick, 1 = pure body).
    pub fn bar_candle_strength(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let range = (b.high - b.low).to_f64()?;
    ...
    /// Mean of open/close ratio (open / close) across bars; None if any close is zero.
    pub fn bar_open_close_ratio(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let ratios: Vec<f64> = bars.iter()
            .filter_map(|b| {
                let o = b.open.to_f64()?;
                let c = b.close.to_f64()?;
                if c == 0.0 { return None; }
    ...
    /// Fraction of bars where close > open (bullish bars).
    pub fn bar_close_above_open_fraction(bars: &[OhlcvBar]) -> Option<f64> {
        if bars.is_empty() { return None; }
        let bullish = bars.iter().filter(|b| b.close > b.open).count();
        Some(bullish as f64 / bars.len() as f64)
    }
    ...
    /// Mean of (high - low) across all bars.
    pub fn bar_hl_spread_mean(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let spreads: Vec<f64> = bars.iter()
            .filter_map(|b| {
                let h = b.high.to_f64()?;
                let l = b.low.to_f64()?;
                Some(h - l)
    ...
    pub fn bar_high_gap_mean(bars: &[OhlcvBar]) -> Option<f64> {
        ...
        let highs: Vec<f64> = bars.iter().filter_map(|b| b.high.to_f64()).collect();
        if highs.len() < 2 { return None; }
        let gaps: Vec<f64> = highs.windows(2).map(|w| w[1] - w[0]).collect();
        Some(gaps.iter().sum::<f64>() / gaps.len() as f64)
    }

    // ── round-163 ────────────────────────────────────────────────────────────

    /// Price efficiency: |close - open| / (high - low); None if range=0.
    pub fn bar_price_efficiency(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter()
            .filter_map(|b| {
    ...
    /// Mean of (close - low) / (high - low) across bars; None if any range=0.
    pub fn bar_close_range_pct(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter()
            .filter_map(|b| {
    ...
    /// Mean of close / (high + low + close) across bars.
    pub fn bar_close_range_ratio(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter()
            .filter_map(|b| {
    ...
    /// Mean of |close - open| / volume (price move per unit volume); None if any vol=0.
    pub fn bar_speed_mean(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter()
            .filter_map(|b| {
    ...
    /// Mean of HLC3 = (high + low + close) / 3 across bars.
    pub fn bar_hlc3_mean(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter()
            .filter_map(|b| {
    ...
    /// Mean of OHLC4 = (open + high + low + close) / 4 across bars.
    pub fn bar_ohlc4_mean(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter()
            .filter_map(|b| {
    ...
    /// Mean of midprice = (high + low) / 2 across bars.
    pub fn bar_mid_price(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter()
            .filter_map(|b| {
    ...
    /// Mean of body as fraction of range: |close-open|/(high-low); None if any range=0.
    pub fn bar_body_pct(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter()
            .filter_map(|b| {
    ...
    /// Mean (close - open) / open * 100 net return across bars.
    pub fn bar_net_return_mean(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let o = b.open.to_f64()?;
    ...
    /// Ratio of up-bar volume to down-bar volume.
    pub fn bar_up_down_vol_ratio(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let mut up_vol = 0.0_f64; let mut down_vol = 0.0_f64;
        for b in bars {
            let o = b.open.to_f64()?;
            let c = b.close.to_f64()?;
            let v = b.volume.to_f64()?;
            if c > o { up_vol += v; } else if c < o { down_vol += v; }
        }
        if down_vol == 0.0 { return None; }
        Some(up_vol / down_vol)
    }

    /// Mean gap between current open and previous close.
    pub fn bar_close_prev_open_gap(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 2 { return None; }
        let vals: Vec<f64> = bars.windows(2).filter_map(|w| {
            let prev_c = w[0].close.to_f64()?;
    ...
    /// Weighted moving average of close prices (linearly weighted, recent = higher).
    pub fn bar_wma_close(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let mut wsum = 0.0_f64; let mut wt = 0.0_f64;
    ...
    /// Mean (open + close) / 2 per bar — bar midpoint price.
    pub fn bar_open_close_midpoint(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let o = b.open.to_f64()?;
    ...
    /// Mean (upper_shadow + lower_shadow) / (high - low) per bar.
    pub fn bar_shadow_to_range(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let h = b.high.to_f64()?;
            ...
            if range == 0.0 { return None; }
            let body = (c - o).abs();
            let shadows = (range - body).max(0.0);
            Some(shadows / range)
        }).collect();
        if vals.is_empty() { return None; }
        Some(vals.iter().sum::<f64>() / vals.len() as f64)
    ...
    /// Linearly weighted moving average of body sizes |close - open|.
    pub fn bar_body_wma(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let mut wsum = 0.0_f64; let mut wt = 0.0_f64;
        for (i, b) in bars.iter().enumerate() {
    ...
    /// Pearson correlation between close and open across bars.
    pub fn bar_close_open_corr(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 2 { return None; }
        let opens: Vec<f64> = bars.iter().filter_map(|b| b.open.to_f64()).collect();
        let closes: Vec<f64> = bars.iter().filter_map(|b| b.close.to_f64()).collect();
        if opens.len() != bars.len() || closes.len() != bars.len() { return None; }
        let n = opens.len() as f64;
        let mo = opens.iter().sum::<f64>() / n;
        let mc = closes.iter().sum::<f64>() / n;
        let num: f64 = opens.iter().zip(closes.iter()).map(|(o, c)| (o - mo) * (c - mc)).sum();
        let so: f64 = opens.iter().map(|o| (o - mo).powi(2)).sum::<f64>().sqrt();
        let sc: f64 = closes.iter().map(|c| (c - mc).powi(2)).sum::<f64>().sqrt();
        if so == 0.0 || sc == 0.0 { return None; }
        Some(num / (so * sc))
    }

    /// Pearson correlation between volume and (high - low) range.
    pub fn bar_volume_range_corr(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 2 { return None; }
        let vols: Vec<f64> = bars.iter().filter_map(|b| b.volume.to_f64()).collect();
        let ranges: Vec<f64> = bars.iter().filter_map(|b| {
        ...
        if vols.len() != bars.len() || ranges.len() != bars.len() { return None; }
        let n = vols.len() as f64;
        let mv = vols.iter().sum::<f64>() / n;
        let mr = ranges.iter().sum::<f64>() / n;
        let num: f64 = vols.iter().zip(ranges.iter()).map(|(v, r)| (v - mv) * (r - mr)).sum();
        let sv: f64 = vols.iter().map(|v| (v - mv).powi(2)).sum::<f64>().sqrt();
        let sr: f64 = ranges.iter().map(|r| (r - mr).powi(2)).sum::<f64>().sqrt();
        if sv == 0.0 || sr == 0.0 { return None; }
        Some(num / (sv * sr))
    }

    /// Mean open / volume per bar — open price per unit volume.
    pub fn bar_open_vol_ratio(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let o = b.open.to_f64()?;
    ...
    /// Mean second difference of close prices: close price acceleration.
    pub fn bar_close_trend_accel(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 3 { return None; }
        let closes: Vec<f64> = bars.iter().filter_map(|b| b.close.to_f64()).collect();
        if closes.len() < 3 { return None; }
        let accels: Vec<f64> = closes.windows(3).map(|w| w[2] - 2.0 * w[1] + w[0]).collect();
        Some(accels.iter().sum::<f64>() / accels.len() as f64)
    }

    /// Fraction of bars where close is above previous close: net uptrend strength.
    pub fn bar_net_trend_strength(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 2 { return None; }
        let ups = bars.windows(2).filter(|w| {
            let pc = w[0].close.to_f64().unwrap_or(0.0);
            let cc = w[1].close.to_f64().unwrap_or(0.0);
            cc > pc
        }).count();
    ...
    pub fn bar_open_gap_pct(bars: &[OhlcvBar]) -> Option<f64> {
        ...
        let vals: Vec<f64> = bars.windows(2).filter_map(|w| {
            ...
            Some((open - prev_close).abs() / prev_close)
        }).collect();
        if vals.is_empty() { return None; }
        Some(vals.iter().sum::<f64>() / vals.len() as f64)
    }
    ...
    pub fn bar_abs_close_momentum(bars: &[OhlcvBar]) -> Option<f64> {
        ...
        if bars.len() < 2 { return None; }
        let vals: Vec<f64> = bars.windows(2).filter_map(|w| {
            Some((w[1].close.to_f64()? - w[0].close.to_f64()?).abs())
        }).collect();
        if vals.is_empty() { return None; }
        Some(vals.iter().sum::<f64>() / vals.len() as f64)
    }
    ...
    pub fn bar_volume_momentum(bars: &[OhlcvBar]) -> Option<f64> {
        ...
        if bars.len() < 2 { return None; }
        let vals: Vec<f64> = bars.windows(2).filter_map(|w| {
            Some(w[1].volume.to_f64()? - w[0].volume.to_f64()?)
        }).collect();
        if vals.is_empty() { return None; }
        Some(vals.iter().sum::<f64>() / vals.len() as f64)
    }
    ...
    /// Mean (open + high + low + close) / 4 per bar.
    pub fn bar_ohlc_mean(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            Some((b.open.to_f64()? + b.high.to_f64()? + b.low.to_f64()? + b.close.to_f64()?) / 4.0)
        }).collect();
        if vals.is_empty() { return None; }
        Some(vals.iter().sum::<f64>() / vals.len() as f64)
    }

    /// Mean second difference of (close - open) per bar: body acceleration.
    pub fn bar_open_close_accel(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 3 { return None; }
        let bodies: Vec<f64> = bars.iter().filter_map(|b| {
            Some(b.close.to_f64()? - b.open.to_f64()?)
    ...
    /// Z-score of latest close vs. rolling close mean and std.
    pub fn bar_close_vol_zscore(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 2 { return None; }
        let closes: Vec<f64> = bars.iter().filter_map(|b| b.close.to_f64()).collect();
        if closes.len() < 2 { return None; }
        ...
        let std = (closes.iter().map(|c| (c - mean).powi(2)).sum::<f64>() / closes.len() as f64).sqrt();
        if std == 0.0 { return None; }
        Some((*closes.last()? - mean) / std)
    }

    /// Mean (open - low) / (high - low): open position within bar range.
    pub fn bar_open_range_pct(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let o = b.open.to_f64()?;
    ...
    /// Mean volume / close per bar.
    pub fn bar_volume_per_close(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let c = b.close.to_f64()?;
            if c == 0.0 { return None; }
            Some(b.volume.to_f64()? / c)
        }).collect();
        if vals.is_empty() { return None; }
        Some(vals.iter().sum::<f64>() / vals.len() as f64)
    }

    /// Fraction of bars where open > prev close (gap up sign).
    pub fn bar_open_gap_sign(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 2 { return None; }
        let signs: Vec<f64> = bars.windows(2).filter_map(|w| {
            let open = w[1].open.to_f64()?;
            let prev_close = w[0].close.to_f64()?;
            Some(if open > prev_close { 1.0 } else if open < prev_close { -1.0 } else { 0.0 })
        }).collect();
        if signs.is_empty() { return None; }
        Some(signs.iter().sum::<f64>() / signs.len() as f64)
    }

    /// Mean |close - ohlc_mean| per bar: close deviation from bar average.
    pub fn bar_close_to_ohlc_mean(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let o = b.open.to_f64()?;
            ...
            let l = b.low.to_f64()?;
            let c = b.close.to_f64()?;
            let ohlc_mean = (o + h + l + c) / 4.0;
            Some((c - ohlc_mean).abs())
        }).collect();
        if vals.is_empty() { return None; }
        Some(vals.iter().sum::<f64>() / vals.len() as f64)
    }
    ...
    pub fn bar_range_change_sign(bars: &[OhlcvBar]) -> Option<f64> {
        ...
        if bars.len() < 2 { return None; }
        let signs: Vec<f64> = bars.windows(2).filter_map(|w| {
            let r0 = w[0].high.to_f64()? - w[0].low.to_f64()?;
            let r1 = w[1].high.to_f64()? - w[1].low.to_f64()?;
            Some(if r1 > r0 { 1.0 } else if r1 < r0 { -1.0 } else { 0.0 })
        }).collect();
        if signs.is_empty() { return None; }
        Some(signs.iter().sum::<f64>() / signs.len() as f64)
    }
    ...
    /// Mean distance from open to high across bars (normalized by range).
    pub fn bar_open_high_dist(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let range = (b.high - b.low).to_f64()?;
            if range == 0.0 { return Some(0.0); }
            let dist = (b.high - b.open).to_f64()?;
            Some(dist / range)
        }).collect();
        if vals.is_empty() { return None; }
        Some(vals.iter().sum::<f64>() / vals.len() as f64)
    ...
    /// Std dev of body-to-range ratios across bars.
    pub fn bar_body_ratio_std(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 2 { return None; }
        let ratios: Vec<f64> = bars.iter().filter_map(|b| {
            let range = (b.high - b.low).to_f64()?;
        ...
        let var = ratios.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / (ratios.len() - 1) as f64;
        Some(var.sqrt())
    }

    /// Ratio of volume to open-to-high range across bars (mean).
    pub fn bar_vol_vs_open_range(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let vol = b.volume.to_f64()?;
            let rng = (b.high - b.open).abs().to_f64()?;
            if rng == 0.0 { return None; }
            Some(vol / rng)
        }).collect();
        if vals.is_empty() { return None; }
        Some(vals.iter().sum::<f64>() / vals.len() as f64)
    ...
    /// Net directional power: mean signed (close-open)/range.
    pub fn bar_trend_power(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let range = (b.high - b.low).to_f64()?;
            if range == 0.0 { return Some(0.0); }
            let net = (b.close - b.open).to_f64()?;
            Some(net / range)
        }).collect();
        if vals.is_empty() { return None; }
        Some(vals.iter().sum::<f64>() / vals.len() as f64)
    ...
    pub fn bar_vol_weighted_high(bars: &[OhlcvBar]) -> Option<f64> {
        ...
        let total_vol: f64 = bars.iter().filter_map(|b| b.volume.to_f64()).sum();
        if total_vol == 0.0 { return None; }
        let weighted: f64 = bars.iter().filter_map(|b| {
            Some(b.high.to_f64()? * b.volume.to_f64()?)
        }).sum();
        Some(weighted / total_vol)
    }

    /// Volume-weighted mean of low prices (vol-weighted low).
    pub fn bar_vol_weighted_low(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let total_vol: f64 = bars.iter().filter_map(|b| b.volume.to_f64()).sum();
        if total_vol == 0.0 { return None; }
        let weighted: f64 = bars.iter().filter_map(|b| {
            Some(b.low.to_f64()? * b.volume.to_f64()?)
        }).sum();
        Some(weighted / total_vol)
    }

    /// Mean reversion of close: fraction of bars where close reverts toward prior open.
    pub fn bar_close_reversion(bars: &[OhlcvBar]) -> Option<f64> {
        if bars.len() < 2 { return None; }
        let n = bars.len() - 1;
        let reversions = bars.windows(2).filter(|w| {
            // Close moved toward the prior bar's open (opposite direction)
            let prior_dir = w[0].close > w[0].open;
            let curr_dir = w[1].close > w[1].open;
            prior_dir != curr_dir
        }).count();
        Some(reversions as f64 / n as f64)
    }

    /// Mean ratio of body to total wick: |close - open| / (upper_wick + lower_wick).
    pub fn bar_body_vs_wick(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let top = b.open.max(b.close);
            ...
            let total_wick = upper + lower;
            if total_wick == 0.0 { return None; }
            Some(body / total_wick)
        }).collect();
        if vals.is_empty() { return None; }
        Some(vals.iter().sum::<f64>() / vals.len() as f64)
    }

    /// Number of bars with volume > mean + std (volume spike count) as fraction of total.
    pub fn bar_vol_spike_count(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 2 { return None; }
        let vols: Vec<f64> = bars.iter().filter_map(|b| b.volume.to_f64()).collect();
        if vols.len() < 2 { return None; }
        ...
        let spikes = vols.iter().filter(|&&v| v > threshold).count();
        Some(spikes as f64 / vols.len() as f64)
    }

    /// Pearson correlation between open prices and close prices across bars.
    pub fn bar_open_close_corr(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 2 { return None; }
        let opens: Vec<f64> = bars.iter().filter_map(|b| b.open.to_f64()).collect();
        let closes: Vec<f64> = bars.iter().filter_map(|b| b.close.to_f64()).collect();
        if opens.len() < 2 || closes.len() != opens.len() { return None; }
        let n = opens.len() as f64;
        let mean_o = opens.iter().sum::<f64>() / n;
        let mean_c = closes.iter().sum::<f64>() / n;
        let cov: f64 = opens.iter().zip(closes.iter()).map(|(o, c)| (o - mean_o) * (c - mean_c)).sum::<f64>() / n;
    ...
    /// Mean signed return: mean (close_t - close_{t-1}) / close_{t-1} across consecutive bars.
    pub fn bar_return_mean(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 2 { return None; }
        let rocs: Vec<f64> = bars.windows(2).filter_map(|w| {
            let prev = w[0].close.to_f64()?;
            let curr = w[1].close.to_f64()?;
            if prev == 0.0 { return None; }
            Some((curr - prev) / prev)
    ...
    /// Mean (close - low) / range across bars (close position above low as fraction of range).
    pub fn bar_close_to_low_pct(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let range = (b.high - b.low).to_f64()?;
            if range == 0.0 { return Some(0.5); }
            let dist = (b.close - b.low).to_f64()?;
            Some(dist / range)
        }).collect();
        if vals.is_empty() { return None; }
        Some(vals.iter().sum::<f64>() / vals.len() as f64)
    ...
    /// Fraction of bars fully inside prior bar's high-low range (inside bar rate).
    pub fn bar_inside_bar_pct(bars: &[OhlcvBar]) -> Option<f64> {
        if bars.len() < 2 { return None; }
        let n = bars.len() - 1;
        let inside = bars.windows(2).filter(|w| {
            w[1].high <= w[0].high && w[1].low >= w[0].low
        }).count();
        Some(inside as f64 / n as f64)
    }

    /// Fraction of bar range occupied by body above midpoint (upper body dominance).
    pub fn bar_upper_body_pct(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let range = (b.high - b.low).to_f64()?;
            if range == 0.0 { return None; }
            let top = b.open.max(b.close);
            let mid = b.low + (b.high - b.low) / rust_decimal::Decimal::from(2);
            let upper_body = (top - mid).max(rust_decimal::Decimal::ZERO).to_f64()?;
            Some(upper_body / range)
        }).collect();
        if vals.is_empty() { return None; }
        Some(vals.iter().sum::<f64>() / vals.len() as f64)
    }

    /// Mean total wick length (upper + lower shadow) per bar.
    pub fn bar_wicks_total(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let top = b.open.max(b.close);
    ...
    /// Fraction of bars where current close is above previous bar's high (breakout rate).
    pub fn bar_close_above_prev_high(bars: &[OhlcvBar]) -> Option<f64> {
        if bars.len() < 2 { return None; }
        let n = bars.len() - 1;
        let breakouts = bars.windows(2).filter(|w| w[1].close > w[0].high).count();
        Some(breakouts as f64 / n as f64)
    }

    /// Mean price range normalized by opening price (normalized volatility).
    pub fn bar_range_open_ratio(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let range = (b.high - b.low).to_f64()?;
    ...
    pub fn bar_vol_roc(bars: &[OhlcvBar]) -> Option<f64> {
        ...
        let vols: Vec<f64> = bars.iter().filter_map(|b| b.volume.to_f64()).collect();
        if vols.len() < 2 { return None; }
        let rocs: Vec<f64> = vols.windows(2).filter_map(|w| {
    ...
    /// Volume-weighted close price: sum(close * volume) / sum(volume).
    pub fn bar_vol_weighted_close(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let total_vol: f64 = bars.iter().filter_map(|b| b.volume.to_f64()).sum();
        if total_vol == 0.0 { return None; }
    ...
    /// Fraction of bars where high is within 1% of the prior bar's high (high clustering).
    pub fn bar_high_low_ratio(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let h = b.high.to_f64()?;
    ...
    /// Skewness of (close - open) body values across bars (directional body skewness).
    pub fn bar_oc_skewness(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 3 { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| (b.close - b.open).to_f64()).collect();
        if vals.len() < 3 { return None; }
    ...
    pub fn bar_body_high_pct(bars: &[OhlcvBar]) -> Option<f64> {
        ...
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let body = (b.close - b.open).abs().to_f64()?;
            let high = b.high.to_f64()?;
            if high == 0.0 { return None; }
            Some(body / high)
        }).collect();
        if vals.is_empty() { return None; }
        Some(vals.iter().sum::<f64>() / vals.len() as f64)
    ...
    /// Fraction of consecutive bar pairs where volume increases (vol trend sign).
    pub fn bar_vol_trend_sign(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 2 { return None; }
        let ups: usize = bars.windows(2).filter(|w| {
            let v0 = w[0].volume.to_f64().unwrap_or(0.0);
            let v1 = w[1].volume.to_f64().unwrap_or(0.0);
            v1 > v0
        }).count();
        Some(ups as f64 / (bars.len() - 1) as f64)
    }

    /// Mean (high - low) / volume — range per unit of volume traded.
    pub fn bar_range_vol_ratio(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let range = (b.high - b.low).to_f64()?;
    ...
    /// Mean (close - open) / open return per bar (signed directional return).
    pub fn bar_open_close_return(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let open = b.open.to_f64()?;
    ...
    /// Mean (close - low) / (high - low) — close pull (position of close in range).
    pub fn bar_close_pull(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let hl = (b.high - b.low).to_f64()?;
    ...
    /// Mean body / (body + total_wick) — body vs shadow ratio.
    pub fn bar_body_vs_shadow(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let body = (b.close - b.open).abs().to_f64()?;
    ...
    /// Mean volume per price point (high - low) across bars.
    pub fn bar_vol_per_point(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let hl = (b.high - b.low).to_f64()?;
    ...
    pub fn bar_oc_mean_abs(bars: &[OhlcvBar]) -> Option<f64> {
        ...
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            (b.open - b.close).abs().to_f64()
        }).collect();
        if vals.is_empty() { return None; }
        Some(vals.iter().sum::<f64>() / vals.len() as f64)
    }

    /// Mean (close - open) / (high - low) — directional efficiency.
    pub fn bar_range_efficiency(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.is_empty() { return None; }
        let vals: Vec<f64> = bars.iter().filter_map(|b| {
            let hl = (b.high - b.low).to_f64()?;
    ...
    /// Z-score of the last bar's volume relative to window mean and std dev.
    pub fn bar_vol_zscore(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 2 { return None; }
        let vols: Vec<f64> = bars.iter().filter_map(|b| b.volume.to_f64()).collect();
        if vols.len() < 2 { return None; }
    ...
    /// Skewness of close prices across bars (third standardized moment).
    pub fn bar_close_skew(bars: &[OhlcvBar]) -> Option<f64> {
        use rust_decimal::prelude::ToPrimitive;
        if bars.len() < 3 { return None; }
        let closes: Vec<f64> = bars.iter().filter_map(|b| b.close.to_f64()).collect();
        if closes.len() < 3 { return None; }
        ...
        let std = var.sqrt();
        if std == 0.0 { return None; }
        let skew = closes.iter().map(|c| ((c - mean) / std).powi(3)).sum::<f64>() / n;
        Some(skew)
    }
...
mod tests {
    ...
    #[test]
    fn test_bar_close_low_trend_none_for_single() {
        assert!(OhlcvBar::bar_close_low_trend(&[make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105))]).is_none());
    }

    #[test]
    fn test_bar_close_low_trend_increasing() {
        let bars = vec![
            make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(100)),  // c-l=10
            make_ohlcv_bar(dec!(100), dec!(115), dec!(90), dec!(105)),  // c-l=15
            make_ohlcv_bar(dec!(100), dec!(120), dec!(90), dec!(112)),  // c-l=22
        ];
        let t = OhlcvBar::bar_close_low_trend(&bars).unwrap();
        assert!(t > 0.0, "expected positive trend, got {}", t);
    }

    #[test]
    fn test_open_body_skew_none_for_empty() {
        assert!(OhlcvBar::open_body_skew(&[]).is_none());
    }

    #[test]
    fn test_open_body_skew_returns_some() {
        let bars = vec![make_ohlcv_bar(dec!(100), dec!(115), dec!(90), dec!(110))];
        assert!(OhlcvBar::open_body_skew(&bars).is_some());
    }

    #[test]
    fn test_bar_volume_trend_ratio_none_for_single() {
        assert!(OhlcvBar::bar_volume_trend_ratio(&[make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105))]).is_none());
    }

    #[test]
    fn test_bar_volume_trend_ratio_range_valid() {
        let bars = vec![
            make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105)),
            make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105)),
        ];
    ...
    #[test]
    fn test_bar_body_range_ratio_none_for_empty() {
        assert!(OhlcvBar::bar_body_range_ratio(&[]).is_none());
    }

    #[test]
    fn test_bar_body_range_ratio_full_body() {
        // close=high, open=low → body=range → ratio=1.0
    ...
    #[test]
    fn test_bar_close_above_open_ratio_none_for_empty() {
        assert!(OhlcvBar::bar_close_above_open_ratio(&[]).is_none());
    }

    #[test]
    fn test_bar_close_above_open_ratio_all_bullish() {
        let bars = vec![
            make_ohlcv_bar(dec!(90), dec!(110), dec!(85), dec!(105)),
            make_ohlcv_bar(dec!(100), dec!(120), dec!(95), dec!(115)),
        ];
        let r = OhlcvBar::bar_close_above_open_ratio(&bars).unwrap();
        assert!((r - 1.0).abs() < 1e-9, "expected 1.0, got {}", r);
    }

    #[test]
    fn test_bar_high_acceleration_none_for_two_bars() {
        let bars = vec![
            make_ohlcv_bar(dec!(90), dec!(110), dec!(85), dec!(100)),
            make_ohlcv_bar(dec!(100), dec!(120), dec!(95), dec!(110)),
        ];
        assert!(OhlcvBar::bar_high_acceleration(&bars).is_none());
    }

    #[test]
    fn test_bar_high_acceleration_constant_rate() {
        // highs: 10, 20, 30 → diffs: 10, 10 → accels: 0
        let bars = vec![
            make_ohlcv_bar(dec!(5), dec!(10), dec!(1), dec!(8)),
            make_ohlcv_bar(dec!(15), dec!(20), dec!(11), dec!(18)),
            make_ohlcv_bar(dec!(25), dec!(30), dec!(21), dec!(28)),
        ];
        let a = OhlcvBar::bar_high_acceleration(&bars).unwrap();
        assert!((a - 0.0).abs() < 1e-9, "expected 0.0, got {}", a);
    }

    #[test]
    fn test_low_open_spread_none_for_empty() {
        assert!(OhlcvBar::low_open_spread(&[]).is_none());
    }

    #[test]
    fn test_low_open_spread_basic() {
        // open=100, low=90 → spread=10
        let bars = vec![make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105))];
        let s = OhlcvBar::low_open_spread(&bars).unwrap();
        assert!((s - 10.0).abs() < 1e-9, "expected 10.0, got {}", s);
    }

    #[test]
    fn test_close_over_open_none_for_empty() {
        assert!(OhlcvBar::close_over_open(&[]).is_none());
    }

    #[test]
    fn test_close_over_open_basic() {
        // close=110, open=100 → ratio=1.1
    ...
    #[test]
    fn test_bar_close_dispersion_none_for_one() {
        let bars = vec![make_ohlcv_bar(dec!(90), dec!(110), dec!(85), dec!(100))];
        assert!(OhlcvBar::bar_close_dispersion(&bars).is_none());
    }

    #[test]
    fn test_bar_close_dispersion_basic() {
        // closes: 100, 100 → std=0
        let bars = vec![
            make_ohlcv_bar(dec!(90), dec!(110), dec!(85), dec!(100)),
            make_ohlcv_bar(dec!(95), dec!(115), dec!(90), dec!(100)),
        ];
        let d = OhlcvBar::bar_close_dispersion(&bars).unwrap();
        assert!((d - 0.0).abs() < 1e-9, "expected 0.0, got {}", d);
    }

    #[test]
    fn test_bar_open_close_mean_none_for_empty() {
        assert!(OhlcvBar::bar_open_close_mean(&[]).is_none());
    }

    #[test]
    fn test_bar_open_close_mean_basic() {
        // open=90, close=110 → (90+110)/2=100
        let bars = vec![make_ohlcv_bar(dec!(90), dec!(115), dec!(85), dec!(110))];
        let m = OhlcvBar::bar_open_close_mean(&bars).unwrap();
        assert!((m - 100.0).abs() < 1e-9, "expected 100.0, got {}", m);
    }

    #[test]
    fn test_close_gap_from_prior_none_for_one() {
        let bars = vec![make_ohlcv_bar(dec!(90), dec!(110), dec!(85), dec!(100))];
        assert!(OhlcvBar::close_gap_from_prior(&bars).is_none());
    }

    #[test]
    fn test_close_gap_from_prior_basic() {
        // closes: 100, 110 → gap=10
        let bars = vec![
            make_ohlcv_bar(dec!(90), dec!(110), dec!(85), dec!(100)),
            make_ohlcv_bar(dec!(100), dec!(120), dec!(95), dec!(110)),
        ];
        let g = OhlcvBar::close_gap_from_prior(&bars).unwrap();
        assert!((g - 10.0).abs() < 1e-9, "expected 10.0, got {}", g);
    }

    #[test]
    fn test_bar_volume_per_bar_none_for_empty() {
        assert!(OhlcvBar::bar_volume_per_bar(&[]).is_none());
    }

    #[test]
    fn test_bar_volume_per_bar_basic() {
        // make_ohlcv_bar uses volume=1, so mean=1.0
        let bars = vec![
            make_ohlcv_bar(dec!(90), dec!(110), dec!(85), dec!(100)),
            make_ohlcv_bar(dec!(100), dec!(120), dec!(95), dec!(110)),
        ];
    ...
    #[test]
    fn test_bar_range_skew_none_for_two() {
        let bars = vec![
            make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105)),
            make_ohlcv_bar(dec!(105), dec!(115), dec!(95), dec!(110)),
        ];
        assert!(OhlcvBar::bar_range_skew(&bars).is_none());
    }

    #[test]
    fn test_bar_range_skew_symmetric_zero() {
        // identical ranges → skew = None (std=0)
        let bars = vec![
            make_ohlcv_bar(dec!(100), dec!(120), dec!(100), dec!(110)),
            make_ohlcv_bar(dec!(110), dec!(130), dec!(110), dec!(120)),
            make_ohlcv_bar(dec!(120), dec!(140), dec!(120), dec!(130)),
        ];
        assert!(OhlcvBar::bar_range_skew(&bars).is_none());
    }

    #[test]
    fn test_bar_open_mid_fraction_none_for_empty() {
        assert!(OhlcvBar::bar_open_mid_fraction(&[]).is_none());
    }

    #[test]
    fn test_bar_open_mid_fraction_open_at_low() {
        // open=low=90 → fraction=0
        let bars = vec![make_ohlcv_bar(dec!(90), dec!(110), dec!(90), dec!(100))];
        let f = OhlcvBar::bar_open_mid_fraction(&bars).unwrap();
        assert!((f - 0.0).abs() < 1e-9, "expected 0.0, got {}", f);
    }

    #[test]
    fn test_bar_high_close_gap_none_for_empty() {
        assert!(OhlcvBar::bar_high_close_gap(&[]).is_none());
    }

    #[test]
    fn test_bar_high_close_gap_close_at_high() {
        // close=high=110 → gap=0
        let bars = vec![make_ohlcv_bar(dec!(90), dec!(110), dec!(90), dec!(110))];
        let g = OhlcvBar::bar_high_close_gap(&bars).unwrap();
        assert!((g - 0.0).abs() < 1e-9, "expected 0.0, got {}", g);
    }

    #[test]
    fn test_bar_open_trend_none_for_single() {
        let bars = vec![make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105))];
        assert!(OhlcvBar::bar_open_trend(&bars).is_none());
    }

    #[test]
    fn test_bar_open_trend_flat() {
        // same open → slope = 0
        let bars = vec![
            make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105)),
            make_ohlcv_bar(dec!(100), dec!(115), dec!(95), dec!(110)),
        ];
        let t = OhlcvBar::bar_open_trend(&bars).unwrap();
        assert!((t - 0.0).abs() < 1e-9, "expected 0.0, got {}", t);
    }
    ...
    #[test]
    fn test_bar_close_vs_range_mid_none_for_empty() {
        assert!(OhlcvBar::bar_close_vs_range_mid(&[]).is_none());
    }

    #[test]
    fn test_bar_close_vs_range_mid_close_at_mid() {
        // high=110, low=90, mid=100, close=100 → diff=0
        let bars = vec![make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(100))];
        let d = OhlcvBar::bar_close_vs_range_mid(&bars).unwrap();
        assert!((d - 0.0).abs() < 1e-9, "expected 0.0, got {}", d);
    }

    #[test]
    fn test_bar_atr_proxy_none_for_single() {
        let bars = vec![make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105))];
        assert!(OhlcvBar::bar_atr_proxy(&bars).is_none());
    }

    #[test]
    fn test_bar_atr_proxy_positive() {
        let bars = vec![
            make_ohlcv_bar(dec!(90), dec!(110), dec!(85), dec!(100)),
            make_ohlcv_bar(dec!(100), dec!(120), dec!(95), dec!(115)),
        ];
        let atr = OhlcvBar::bar_atr_proxy(&bars).unwrap();
        assert!(atr > 0.0, "expected positive ATR proxy, got {}", atr);
    }

    #[test]
    fn test_bar_mid_close_diff_none_for_empty() {
        assert!(OhlcvBar::bar_mid_close_diff(&[]).is_none());
    }

    #[test]
    fn test_bar_mid_close_diff_close_at_mid() {
        // high=110, low=90 → mid=100, close=100 → diff=0
        let bars = vec![make_ohlcv_bar(dec!(95), dec!(110), dec!(90), dec!(100))];
        let d = OhlcvBar::bar_mid_close_diff(&bars).unwrap();
        assert!((d - 0.0).abs() < 1e-9, "expected 0.0, got {}", d);
    }

    #[test]
    fn test_bar_candle_strength_none_for_empty() {
        assert!(OhlcvBar::bar_candle_strength(&[]).is_none());
    }

    #[test]
    fn test_bar_candle_strength_full_body() {
        // open=low=90, close=high=110 → body=range → strength=1.0
        let bars = vec![make_ohlcv_bar(dec!(90), dec!(110), dec!(90), dec!(110))];
    ...
    #[test]
    fn test_bar_open_close_ratio_none_for_empty() {
        assert!(OhlcvBar::bar_open_close_ratio(&[]).is_none());
    }

    #[test]
    fn test_bar_open_close_ratio_equal_one() {
        // open=close → ratio=1.0
        let bars = vec![make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(100))];
        let r = OhlcvBar::bar_open_close_ratio(&bars).unwrap();
        assert!((r - 1.0).abs() < 1e-9, "expected 1.0, got {}", r);
    }

    #[test]
    fn test_bar_close_above_open_fraction_none_for_empty() {
        assert!(OhlcvBar::bar_close_above_open_fraction(&[]).is_none());
    }

    #[test]
    fn test_bar_close_above_open_fraction_all_bullish_one() {
        let bars = vec![make_ohlcv_bar(dec!(100), dec!(115), dec!(90), dec!(110))];
        let f = OhlcvBar::bar_close_above_open_fraction(&bars).unwrap();
        assert!((f - 1.0).abs() < 1e-9, "expected 1.0, got {}", f);
    }

    #[test]
    fn test_bar_hl_spread_mean_none_for_empty() {
        assert!(OhlcvBar::bar_hl_spread_mean(&[]).is_none());
    }

    #[test]
    fn test_bar_hl_spread_mean_basic() {
        // high=110, low=90 → spread=20
        let bars = vec![make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105))];
        let s = OhlcvBar::bar_hl_spread_mean(&bars).unwrap();
        assert!((s - 20.0).abs() < 1e-9, "expected 20.0, got {}", s);
    }

    #[test]
    fn test_bar_high_gap_mean_none_for_single() {
        let bars = vec![make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105))];
        assert!(OhlcvBar::bar_high_gap_mean(&bars).is_none());
    }

    #[test]
    fn test_bar_high_gap_mean_constant_zero() {
        let bars = vec![
            make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105)),
            make_ohlcv_bar(dec!(105), dec!(110), dec!(95), dec!(108)),
        ];
        let g = OhlcvBar::bar_high_gap_mean(&bars).unwrap();
        assert!((g - 0.0).abs() < 1e-9, "expected 0.0, got {}", g);
    }
    ...
    #[test]
    fn test_bar_price_efficiency_none_for_empty() {
        assert!(OhlcvBar::bar_price_efficiency(&[]).is_none());
    }

    #[test]
    fn test_bar_price_efficiency_full_range_one() {
        // open=90, close=110, high=110, low=90 → efficiency=1.0
        let bars = vec![make_ohlcv_bar(dec!(90), dec!(110), dec!(90), dec!(110))];
        let e = OhlcvBar::bar_price_efficiency(&bars).unwrap();
        assert!((e - 1.0).abs() < 1e-9, "expected 1.0, got {}", e);
    }

    #[test]
    fn test_bar_close_range_pct_none_for_empty() {
        assert!(OhlcvBar::bar_close_range_pct(&[]).is_none());
    }

    #[test]
    fn test_bar_close_range_pct_close_at_high_one() {
        // close=high → (close-low)/(high-low) = 1.0
        let bars = vec![make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(110))];
        let p = OhlcvBar::bar_close_range_pct(&bars).unwrap();
        assert!((p - 1.0).abs() < 1e-9, "expected 1.0, got {}", p);
    }

    #[test]
    fn test_bar_close_range_ratio_none_for_empty() {
        assert!(OhlcvBar::bar_close_range_ratio(&[]).is_none());
    }

    #[test]
    fn test_bar_close_range_ratio_positive() {
        let bars = vec![make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105))];
        let r = OhlcvBar::bar_close_range_ratio(&bars).unwrap();
        // 105 / (110+90+105) = 105/305
        let expected = 105.0 / 305.0;
        assert!((r - expected).abs() < 1e-9, "expected {}, got {}", expected, r);
    }

    #[test]
    fn test_bar_speed_mean_none_for_empty() {
        assert!(OhlcvBar::bar_speed_mean(&[]).is_none());
    }

    #[test]
    fn test_bar_speed_mean_no_move_zero() {
        // open=close → |close-open|=0
        let bars = vec![make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(100))];
        let s = OhlcvBar::bar_speed_mean(&bars).unwrap();
        assert!((s - 0.0).abs() < 1e-9, "expected 0.0, got {}", s);
    }
    ...
    #[test]
    fn test_bar_hlc3_mean_none_for_empty() {
        assert!(OhlcvBar::bar_hlc3_mean(&[]).is_none());
    }

    #[test]
    fn test_bar_hlc3_mean_basic() {
        // high=110, low=90, close=105 → hlc3=101.67
        let bars = vec![make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105))];
        let m = OhlcvBar::bar_hlc3_mean(&bars).unwrap();
        let expected = (110.0 + 90.0 + 105.0) / 3.0;
        assert!((m - expected).abs() < 1e-9, "expected {}, got {}", expected, m);
    }

    #[test]
    fn test_bar_ohlc4_mean_none_for_empty() {
        assert!(OhlcvBar::bar_ohlc4_mean(&[]).is_none());
    }

    #[test]
    fn test_bar_ohlc4_mean_basic() {
        // open=100, high=110, low=90, close=105 → ohlc4=101.25
        let bars = vec![make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105))];
        let m = OhlcvBar::bar_ohlc4_mean(&bars).unwrap();
        let expected = (100.0 + 110.0 + 90.0 + 105.0) / 4.0;
        assert!((m - expected).abs() < 1e-9, "expected {}, got {}", expected, m);
    }

    #[test]
    fn test_bar_mid_price_none_for_empty() {
        assert!(OhlcvBar::bar_mid_price(&[]).is_none());
    }

    #[test]
    fn test_bar_mid_price_basic() {
        // high=110, low=90 → mid=100
        let bars = vec![make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105))];
        let m = OhlcvBar::bar_mid_price(&bars).unwrap();
        assert!((m - 100.0).abs() < 1e-9, "expected 100.0, got {}", m);
    }

    #[test]
    fn test_bar_body_pct_none_for_empty() {
        assert!(OhlcvBar::bar_body_pct(&[]).is_none());
    }

    #[test]
    fn test_bar_body_pct_full_body_one() {
        // open=90, close=110, high=110, low=90 → body=range → pct=1.0
        let bars = vec![make_ohlcv_bar(dec!(90), dec!(110), dec!(90), dec!(110))];
    ...
    #[test]
    fn test_bar_net_return_mean_none_for_empty() {
        assert!(OhlcvBar::bar_net_return_mean(&[]).is_none());
    }

    #[test]
    fn test_bar_net_return_mean_flat_zero() {
        // open=close=100 → return=0
        let bar = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(100));
        let r = OhlcvBar::bar_net_return_mean(&[bar]).unwrap();
        assert!(r.abs() < 1e-9, "expected 0.0, got {}", r);
    }

    #[test]
    fn test_bar_up_down_vol_ratio_none_for_empty() {
        assert!(OhlcvBar::bar_up_down_vol_ratio(&[]).is_none());
    }

    #[test]
    fn test_bar_up_down_vol_ratio_no_down_none() {
        // All bullish → down_vol=0 → None
        let bar = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_up_down_vol_ratio(&[bar]).is_none());
    }

    #[test]
    fn test_bar_close_prev_open_gap_none_for_empty() {
        assert!(OhlcvBar::bar_close_prev_open_gap(&[]).is_none());
    }

    #[test]
    fn test_bar_close_prev_open_gap_none_for_single() {
        let bar = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(100));
        assert!(OhlcvBar::bar_close_prev_open_gap(&[bar]).is_none());
    }

    #[test]
    fn test_bar_close_prev_open_gap_no_gap() {
        // bar0 close=100, bar1 open=100 → gap=0
        let b0 = make_ohlcv_bar(dec!(95), dec!(110), dec!(90), dec!(100));
        let b1 = make_ohlcv_bar(dec!(100), dec!(115), dec!(95), dec!(105));
        let r = OhlcvBar::bar_close_prev_open_gap(&[b0, b1]).unwrap();
        assert!(r.abs() < 1e-9, "expected 0.0, got {}", r);
    }

    #[test]
    fn test_bar_wma_close_none_for_empty() {
        assert!(OhlcvBar::bar_wma_close(&[]).is_none());
    }

    #[test]
    fn test_bar_wma_close_single_bar() {
        // Single bar with close=100 → wma=100
    ...
    #[test]
    fn test_bar_open_close_midpoint_none_for_empty() {
        assert!(OhlcvBar::bar_open_close_midpoint(&[]).is_none());
    }

    #[test]
    fn test_bar_open_close_midpoint_open_equals_close() {
        // O=C=100 → midpoint=100
        let bar = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(100));
        let r = OhlcvBar::bar_open_close_midpoint(&[bar]).unwrap();
        assert!((r - 100.0).abs() < 1e-9, "expected 100.0, got {}", r);
    }

    #[test]
    fn test_bar_shadow_to_range_none_for_empty() {
        assert!(OhlcvBar::bar_shadow_to_range(&[]).is_none());
    }

    #[test]
    fn test_bar_shadow_to_range_full_body_zero() {
        // O=L, C=H → body=range → shadows=0 → ratio=0
        let bar = make_ohlcv_bar(dec!(90), dec!(110), dec!(90), dec!(110));
        let r = OhlcvBar::bar_shadow_to_range(&[bar]).unwrap();
        assert!(r.abs() < 1e-9, "expected 0.0, got {}", r);
    }

    #[test]
    fn test_bar_body_wma_none_for_empty() {
        assert!(OhlcvBar::bar_body_wma(&[]).is_none());
    }

    #[test]
    fn test_bar_body_wma_single_bar() {
        // body = |close - open| = |105 - 100| = 5
        let bar = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let r = OhlcvBar::bar_body_wma(&[bar]).unwrap();
        assert!((r - 5.0).abs() < 1e-9, "expected 5.0, got {}", r);
    }

    #[test]
    fn test_bar_close_open_corr_none_for_empty() {
        assert!(OhlcvBar::bar_close_open_corr(&[]).is_none());
    }

    #[test]
    fn test_bar_close_open_corr_single_bar_none() {
        let bar = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_close_open_corr(&[bar]).is_none());
    }

    #[test]
    fn test_bar_close_open_corr_constant_none() {
        // All same → std=0 → None
        let b0 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_close_open_corr(&[b0, b1]).is_none());
    }

    // --- round-180 ---

    #[test]
    fn test_bar_volume_range_corr_none_for_empty() {
        assert!(OhlcvBar::bar_volume_range_corr(&[]).is_none());
    }

    #[test]
    fn test_bar_volume_range_corr_single_bar_none() {
        let bar = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_volume_range_corr(&[bar]).is_none());
    }

    #[test]
    fn test_bar_open_vol_ratio_none_for_empty() {
        assert!(OhlcvBar::bar_open_vol_ratio(&[]).is_none());
    }

    #[test]
    fn test_bar_open_vol_ratio_basic() {
        // O=100, V=1 → ratio=100
        let bar = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let r = OhlcvBar::bar_open_vol_ratio(&[bar]).unwrap();
        assert!((r - 100.0).abs() < 1e-9, "expected 100.0, got {}", r);
    }

    #[test]
    fn test_bar_close_trend_accel_none_for_short() {
        let b0 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(100));
        let b1 = make_ohlcv_bar(dec!(101), dec!(111), dec!(91), dec!(101));
        assert!(OhlcvBar::bar_close_trend_accel(&[b0, b1]).is_none());
    }

    #[test]
    fn test_bar_close_trend_accel_linear_zero() {
        // Linear closes → accel = 0
        let b0 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(100));
        let b1 = make_ohlcv_bar(dec!(101), dec!(111), dec!(91), dec!(101));
        let b2 = make_ohlcv_bar(dec!(102), dec!(112), dec!(92), dec!(102));
        let r = OhlcvBar::bar_close_trend_accel(&[b0, b1, b2]).unwrap();
        assert!(r.abs() < 1e-9, "expected 0.0, got {}", r);
    }

    #[test]
    fn test_bar_net_trend_strength_none_for_empty() {
        assert!(OhlcvBar::bar_net_trend_strength(&[]).is_none());
    }

    #[test]
    fn test_bar_net_trend_strength_all_up_one() {
        // Each bar's close > prev close
        let b0 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(100));
        let b1 = make_ohlcv_bar(dec!(100), dec!(115), dec!(95), dec!(105));
    ...
    #[test]
    fn test_bar_open_gap_pct_none_for_single() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_open_gap_pct(&[b]).is_none());
    }

    #[test]
    fn test_bar_open_gap_pct_no_gap_zero() {
        // b0 close=100, b1 open=100 → gap=0
        let b0 = make_ohlcv_bar(dec!(95), dec!(110), dec!(90), dec!(100));
        let b1 = make_ohlcv_bar(dec!(100), dec!(115), dec!(95), dec!(110));
        let r = OhlcvBar::bar_open_gap_pct(&[b0, b1]).unwrap();
        assert!(r.abs() < 1e-9, "expected 0.0, got {}", r);
    }

    #[test]
    fn test_bar_abs_close_momentum_none_for_single() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_abs_close_momentum(&[b]).is_none());
    }

    #[test]
    fn test_bar_abs_close_momentum_value() {
        // close: 100, 105 → |105-100|=5
        let b0 = make_ohlcv_bar(dec!(95), dec!(110), dec!(90), dec!(100));
        let b1 = make_ohlcv_bar(dec!(100), dec!(115), dec!(95), dec!(105));
        let r = OhlcvBar::bar_abs_close_momentum(&[b0, b1]).unwrap();
        assert!((r - 5.0).abs() < 1e-9, "expected 5.0, got {}", r);
    }

    #[test]
    fn test_bar_volume_momentum_none_for_single() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_volume_momentum(&[b]).is_none());
    }

    #[test]
    fn test_bar_close_level_return_none_for_empty() {
        assert!(OhlcvBar::bar_close_level_return(&[]).is_none());
    }

    #[test]
    fn test_bar_close_level_return_breakeven_zero() {
        // open=close → return=0
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(100));
    ...
    #[test]
    fn test_bar_ohlc_mean_single() {
        // (100+110+90+105)/4 = 101.25
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let r = OhlcvBar::bar_ohlc_mean(&[b]).unwrap();
        assert!((r - 101.25).abs() < 1e-6, "expected 101.25, got {}", r);
    }

    #[test]
    fn test_bar_ohlc_mean_empty_none() {
        assert!(OhlcvBar::bar_ohlc_mean(&[]).is_none());
    }

    #[test]
    fn test_bar_open_close_accel_none_for_short() {
        let b0 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b1 = make_ohlcv_bar(dec!(105), dec!(115), dec!(95), dec!(110));
        assert!(OhlcvBar::bar_open_close_accel(&[b0, b1]).is_none());
    }

    #[test]
    fn test_bar_open_close_accel_constant_body_zero() {
        // Each bar: close-open=5 → linear → accel=0
        let b0 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b1 = make_ohlcv_bar(dec!(105), dec!(115), dec!(95), dec!(110));
        let b2 = make_ohlcv_bar(dec!(110), dec!(120), dec!(100), dec!(115));
        let r = OhlcvBar::bar_open_close_accel(&[b0, b1, b2]).unwrap();
        assert!(r.abs() < 1e-9, "expected 0.0, got {}", r);
    }

    #[test]
    fn test_bar_close_vol_zscore_none_for_constant() {
        // All same close → std=0 → None
        let b0 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b1 = make_ohlcv_bar(dec!(105), dec!(115), dec!(95), dec!(105));
        assert!(OhlcvBar::bar_close_vol_zscore(&[b0, b1]).is_none());
    }

    #[test]
    fn test_bar_open_range_pct_empty_none() {
        assert!(OhlcvBar::bar_open_range_pct(&[]).is_none());
    }

    #[test]
    fn test_bar_open_range_pct_open_at_low_zero() {
        // open=low → (open-low)/(high-low) = 0
        let b = make_ohlcv_bar(dec!(90), dec!(110), dec!(90), dec!(100));
        let r = OhlcvBar::bar_open_range_pct(&[b]).unwrap();
        assert!(r.abs() < 1e-9, "expected 0.0, got {}", r);
    }

    #[test]
    fn test_bar_volume_per_close_empty_none() {
        assert!(OhlcvBar::bar_volume_per_close(&[]).is_none());
    }

    #[test]
    fn test_bar_volume_per_close_value() {
        // volume=1, close=100 → 1/100=0.01
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(100));
        let r = OhlcvBar::bar_volume_per_close(&[b]).unwrap();
        assert!((r - 0.01).abs() < 1e-9, "expected 0.01, got {}", r);
    }

    #[test]
    fn test_bar_open_gap_sign_none_for_single() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_open_gap_sign(&[b]).is_none());
    }

    #[test]
    fn test_bar_open_gap_sign_no_gap_zero() {
        // b0 close=105, b1 open=105 → sign=0
        let b0 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b1 = make_ohlcv_bar(dec!(105), dec!(115), dec!(95), dec!(110));
        let r = OhlcvBar::bar_open_gap_sign(&[b0, b1]).unwrap();
        assert!(r.abs() < 1e-9, "expected 0.0, got {}", r);
    }

    #[test]
    fn test_bar_close_to_ohlc_mean_empty_none() {
        assert!(OhlcvBar::bar_close_to_ohlc_mean(&[]).is_none());
    }

    #[test]
    fn test_bar_close_to_ohlc_mean_symmetric_zero() {
        // open=100,high=110,low=90,close=100 → ohlc_mean=100 → |100-100|=0
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(100));
        let r = OhlcvBar::bar_close_to_ohlc_mean(&[b]).unwrap();
        assert!(r.abs() < 1e-9, "expected 0.0, got {}", r);
    }

    #[test]
    fn test_bar_range_change_sign_none_for_single() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_range_change_sign(&[b]).is_none());
    }

    #[test]
    fn test_bar_range_change_sign_constant_range_zero() {
        // Both bars have same range=20 → sign=0
        let b0 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b1 = make_ohlcv_bar(dec!(105), dec!(115), dec!(95), dec!(110));
    ...
    #[test]
    fn test_bar_open_high_dist_empty_none() {
        assert!(OhlcvBar::bar_open_high_dist(&[]).is_none());
    }

    #[test]
    fn test_bar_open_high_dist_returns_fraction() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let r = OhlcvBar::bar_open_high_dist(&[b]).unwrap();
        // (110-100)/(110-90) = 10/20 = 0.5
        assert!((r - 0.5).abs() < 1e-9);
    }

    #[test]
    fn test_bar_body_ratio_std_too_few_none() {
        assert!(OhlcvBar::bar_body_ratio_std(&[]).is_none());
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_body_ratio_std(&[b]).is_none());
    }

    #[test]
    fn test_bar_body_ratio_std_returns_value() {
        let b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b2 = make_ohlcv_bar(dec!(100), dec!(120), dec!(80), dec!(110));
        let r = OhlcvBar::bar_body_ratio_std(&[b1, b2]);
        assert!(r.is_some());
        assert!(r.unwrap() >= 0.0);
    }

    #[test]
    fn test_bar_vol_vs_open_range_empty_none() {
        assert!(OhlcvBar::bar_vol_vs_open_range(&[]).is_none());
    }

    #[test]
    fn test_bar_vol_vs_open_range_returns_value() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let r = OhlcvBar::bar_vol_vs_open_range(&[b]);
        assert!(r.is_some());
        assert!(r.unwrap() > 0.0);
    }

    #[test]
    fn test_bar_trend_power_empty_none() {
        assert!(OhlcvBar::bar_trend_power(&[]).is_none());
    }

    #[test]
    fn test_bar_trend_power_bullish() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(110));
    ...
    #[test]
    fn test_bar_trend_power_bearish() {
        let b = make_ohlcv_bar(dec!(110), dec!(120), dec!(90), dec!(90));
        let r = OhlcvBar::bar_trend_power(&[b]).unwrap();
        assert!(r < 0.0);
    }
    ...
    #[test]
    fn test_bar_oc_mean_abs_empty_none() {
        assert!(OhlcvBar::bar_oc_mean_abs(&[]).is_none());
    }

    #[test]
    fn test_bar_oc_mean_abs_returns_nonneg() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let r = OhlcvBar::bar_oc_mean_abs(&[b]).unwrap();
        assert!(r >= 0.0);
    }

    #[test]
    fn test_bar_range_efficiency_empty_none() {
        assert!(OhlcvBar::bar_range_efficiency(&[]).is_none());
    }

    #[test]
    fn test_bar_range_efficiency_bullish_positive() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(108));
        let r = OhlcvBar::bar_range_efficiency(&[b]).unwrap();
        assert!(r > 0.0);
    }

    #[test]
    fn test_bar_vol_zscore_too_few_none() {
        assert!(OhlcvBar::bar_vol_zscore(&[]).is_none());
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_vol_zscore(&[b]).is_none());
    }

    #[test]
    fn test_bar_vol_zscore_returns_value() {
        let b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b2 = make_ohlcv_bar(dec!(100), dec!(115), dec!(85), dec!(110));
        // same volume (dec!(1)) → std=0 → None
        assert!(OhlcvBar::bar_vol_zscore(&[b1, b2]).is_none());
    }

    #[test]
    fn test_bar_close_skew_too_few_none() {
        assert!(OhlcvBar::bar_close_skew(&[]).is_none());
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_close_skew(&[b]).is_none());
    }

    #[test]
    fn test_bar_close_skew_symmetric_near_zero() {
        let b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(95));
        let b2 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(100));
    ...
    #[test]
    fn test_bar_open_strength_empty_none() {
        assert!(OhlcvBar::bar_open_strength(&[]).is_none());
    }
    ...
    #[test]
    fn test_bar_close_pull_empty_none() {
        assert!(OhlcvBar::bar_close_pull(&[]).is_none());
    }
    ...
    #[test]
    fn test_bar_body_vs_shadow_empty_none() {
        assert!(OhlcvBar::bar_body_vs_shadow(&[]).is_none());
    }
    ...
    #[test]
    fn test_bar_vol_per_point_empty_none() {
        assert!(OhlcvBar::bar_vol_per_point(&[]).is_none());
    }

    #[test]
    fn test_bar_vol_per_point_returns_positive() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
    ...
    #[test]
    fn test_bar_body_high_pct_empty_none() {
        assert!(OhlcvBar::bar_body_high_pct(&[]).is_none());
    }

    #[test]
    fn test_bar_body_high_pct_bounded() {
        let b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b2 = make_ohlcv_bar(dec!(105), dec!(115), dec!(95), dec!(110));
    ...
    #[test]
    fn test_bar_vol_trend_sign_too_few_none() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_vol_trend_sign(&[b]).is_none());
    }

    #[test]
    fn test_bar_vol_trend_sign_returns_value() {
        let b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b2 = make_ohlcv_bar(dec!(105), dec!(115), dec!(95), dec!(110));
        let b3 = make_ohlcv_bar(dec!(110), dec!(120), dec!(100), dec!(115));
        let r = OhlcvBar::bar_vol_trend_sign(&[b1, b2, b3]);
        assert!(r.is_some());
    }

    #[test]
    fn test_bar_range_vol_ratio_empty_none() {
        assert!(OhlcvBar::bar_range_vol_ratio(&[]).is_none());
    }

    #[test]
    fn test_bar_range_vol_ratio_nonneg() {
        let b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b2 = make_ohlcv_bar(dec!(105), dec!(120), dec!(95), dec!(115));
        let r = OhlcvBar::bar_range_vol_ratio(&[b1, b2]).unwrap();
        assert!(r >= 0.0);
    }

    #[test]
    fn test_bar_open_close_return_empty_none() {
        assert!(OhlcvBar::bar_open_close_return(&[]).is_none());
    }

    #[test]
    fn test_bar_open_close_return_returns_value() {
        let b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
    ...
    #[test]
    fn test_bar_vol_roc_too_few_none() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_vol_roc(&[b]).is_none());
    }

    #[test]
    fn test_bar_vol_roc_returns_value() {
        let b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b2 = make_ohlcv_bar(dec!(105), dec!(115), dec!(95), dec!(110));
        let r = OhlcvBar::bar_vol_roc(&[b1, b2]);
        // Both bars have volume=dec!(1) so roc = 0
        assert_eq!(r.unwrap(), 0.0);
    }

    #[test]
    fn test_bar_vol_weighted_close_empty_none() {
        assert!(OhlcvBar::bar_vol_weighted_close(&[]).is_none());
    }

    #[test]
    fn test_bar_vol_weighted_close_returns_value() {
        let b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b2 = make_ohlcv_bar(dec!(105), dec!(115), dec!(95), dec!(110));
        let r = OhlcvBar::bar_vol_weighted_close(&[b1, b2]).unwrap();
        assert!((r - 107.5).abs() < 1e-6);
    }

    #[test]
    fn test_bar_high_low_ratio_empty_none() {
        assert!(OhlcvBar::bar_high_low_ratio(&[]).is_none());
    }

    #[test]
    fn test_bar_high_low_ratio_nonneg() {
        let b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b2 = make_ohlcv_bar(dec!(100), dec!(115), dec!(85), dec!(110));
        let r = OhlcvBar::bar_high_low_ratio(&[b1, b2]).unwrap();
        assert!(r >= 1.0); // high/low >= 1
    }

    #[test]
    fn test_bar_oc_skewness_too_few_none() {
        let b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b2 = make_ohlcv_bar(dec!(100), dec!(115), dec!(85), dec!(98));
        assert!(OhlcvBar::bar_oc_skewness(&[b1, b2]).is_none());
    }

    #[test]
    fn test_bar_oc_skewness_returns_value() {
        let b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b2 = make_ohlcv_bar(dec!(100), dec!(115), dec!(85), dec!(98));
        let b3 = make_ohlcv_bar(dec!(100), dec!(112), dec!(88), dec!(108));
    ...
    #[test]
    fn test_bar_wicks_total_empty_none() {
        assert!(OhlcvBar::bar_wicks_total(&[]).is_none());
    }

    #[test]
    fn test_bar_wicks_total_returns_nonneg() {
        // open=100, close=105, high=112, low=90
        // upper=112-105=7, lower=100-90=10, total=17
        let b = make_ohlcv_bar(dec!(100), dec!(112), dec!(90), dec!(105));
        let r = OhlcvBar::bar_wicks_total(&[b]).unwrap();
        assert!((r - 17.0).abs() < 1e-9, "expected 17.0 got {}", r);
    }

    #[test]
    fn test_bar_close_above_prev_high_too_few_none() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_close_above_prev_high(&[b]).is_none());
    }

    #[test]
    fn test_bar_close_above_prev_high_returns_bounded() {
        let b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b2 = make_ohlcv_bar(dec!(105), dec!(115), dec!(95), dec!(120)); // close=120 > high=110
        let r = OhlcvBar::bar_close_above_prev_high(&[b1, b2]).unwrap();
        assert!((r - 1.0).abs() < 1e-9, "expected 1.0 got {}", r);
    }

    #[test]
    fn test_bar_range_open_ratio_empty_none() {
        assert!(OhlcvBar::bar_range_open_ratio(&[]).is_none());
    }

    #[test]
    fn test_bar_range_open_ratio_returns_nonneg() {
        // range=20, open=100 → 0.2
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let r = OhlcvBar::bar_range_open_ratio(&[b]).unwrap();
        assert!((r - 0.2).abs() < 1e-9, "expected 0.2 got {}", r);
    }

    #[test]
    fn test_bar_vol_consistency_too_few_none() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_vol_consistency(&[b]).is_none());
    }

    #[test]
    fn test_bar_vol_consistency_equal_volumes_zero() {
        let mut b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        b1.volume = dec!(10);
        let mut b2 = make_ohlcv_bar(dec!(105), dec!(115), dec!(95), dec!(110));
        b2.volume = dec!(10);
        let r = OhlcvBar::bar_vol_consistency(&[b1, b2]).unwrap();
        assert!(r.abs() < 1e-9, "expected 0 got {}", r);
    }

    #[test]
    fn test_bar_close_to_low_pct_empty_none() {
        assert!(OhlcvBar::bar_close_to_low_pct(&[]).is_none());
    }

    #[test]
    fn test_bar_close_to_low_pct_returns_bounded() {
        // close=low → 0.0
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(90));
        let r = OhlcvBar::bar_close_to_low_pct(&[b]).unwrap();
        assert!(r.abs() < 1e-9, "expected 0 got {}", r);
    }

    #[test]
    fn test_bar_inside_bar_pct_too_few_none() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_inside_bar_pct(&[b]).is_none());
    }

    #[test]
    fn test_bar_inside_bar_pct_inside_returns_one() {
        let b1 = make_ohlcv_bar(dec!(100), dec!(120), dec!(80), dec!(110));
        let b2 = make_ohlcv_bar(dec!(105), dec!(115), dec!(85), dec!(110)); // fully inside b1
        let r = OhlcvBar::bar_inside_bar_pct(&[b1, b2]).unwrap();
        assert!((r - 1.0).abs() < 1e-9, "expected 1.0 got {}", r);
    }

    #[test]
    fn test_bar_upper_body_pct_empty_none() {
        assert!(OhlcvBar::bar_upper_body_pct(&[]).is_none());
    }
    ...
    #[test]
    fn test_bar_vol_spike_count_too_few_none() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_vol_spike_count(&[b]).is_none());
    }

    #[test]
    fn test_bar_vol_spike_count_returns_bounded() {
        let mut b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        b1.volume = dec!(5);
        let mut b2 = make_ohlcv_bar(dec!(105), dec!(115), dec!(95), dec!(110));
        b2.volume = dec!(100); // spike
    ...
    #[test]
    fn test_bar_open_close_corr_too_few_none() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_open_close_corr(&[b]).is_none());
    }

    #[test]
    fn test_bar_open_close_corr_returns_bounded() {
        let b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        let b2 = make_ohlcv_bar(dec!(105), dec!(115), dec!(95), dec!(112));
    ...
    #[test]
    fn test_bar_return_mean_too_few_none() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_return_mean(&[b]).is_none());
    }

    #[test]
    fn test_bar_return_mean_returns_value() {
        let b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(100));
        let b2 = make_ohlcv_bar(dec!(100), dec!(115), dec!(95), dec!(110));
        // return = (110-100)/100 = 0.1
        let r = OhlcvBar::bar_return_mean(&[b1, b2]).unwrap();
        assert!((r - 0.1).abs() < 1e-9, "expected 0.1 got {}", r);
    }

    #[test]
    fn test_bar_vol_weighted_high_empty_none() {
        assert!(OhlcvBar::bar_vol_weighted_high(&[]).is_none());
    }

    #[test]
    fn test_bar_vol_weighted_high_equal_vol_returns_mean() {
        let mut b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        b1.volume = dec!(1);
        let mut b2 = make_ohlcv_bar(dec!(105), dec!(115), dec!(95), dec!(110));
        b2.volume = dec!(1);
        // equal vol: vwh = (110+115)/2 = 112.5
        let r = OhlcvBar::bar_vol_weighted_high(&[b1, b2]).unwrap();
        assert!((r - 112.5).abs() < 1e-9, "expected 112.5 got {}", r);
    }

    #[test]
    fn test_bar_vol_weighted_low_empty_none() {
        assert!(OhlcvBar::bar_vol_weighted_low(&[]).is_none());
    }

    #[test]
    fn test_bar_vol_weighted_low_equal_vol_returns_mean() {
        let mut b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        b1.volume = dec!(1);
        let mut b2 = make_ohlcv_bar(dec!(105), dec!(115), dec!(95), dec!(110));
        b2.volume = dec!(1);
        // equal vol: vwl = (90+95)/2 = 92.5
        let r = OhlcvBar::bar_vol_weighted_low(&[b1, b2]).unwrap();
        assert!((r - 92.5).abs() < 1e-9, "expected 92.5 got {}", r);
    }

    #[test]
    fn test_bar_close_reversion_too_few_none() {
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
        assert!(OhlcvBar::bar_close_reversion(&[b]).is_none());
    }

    #[test]
    fn test_bar_close_reversion_returns_bounded() {
        let b1 = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105)); // bull
        let b2 = make_ohlcv_bar(dec!(105), dec!(115), dec!(95), dec!(100)); // bear
        let r = OhlcvBar::bar_close_reversion(&[b1, b2]).unwrap();
        assert!((r - 1.0).abs() < 1e-9, "expected 1.0 got {}", r);
    }

    #[test]
    fn test_bar_body_vs_wick_empty_none() {
        assert!(OhlcvBar::bar_body_vs_wick(&[]).is_none());
    }

    #[test]
    fn test_bar_body_vs_wick_returns_nonneg() {
        // open=100, close=105, high=110, low=90 → body=5, upper=5, lower=10, total_wick=15
        let b = make_ohlcv_bar(dec!(100), dec!(110), dec!(90), dec!(105));
